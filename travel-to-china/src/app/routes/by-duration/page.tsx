...
export async function generateMetadata() {
  const entry = getContentBySlug('routes/by-duration', 'index');
  if (!entry) return {};
  return {
...
export default function ByDurationPage() {
  const entry = getContentBySlug('routes/by-duration', 'index');
  if (!entry) notFound();

  const related = getRelatedArticles('index', 'routes' as any, 3);
  const breadcrumbs = [
    { label: 'Home', href: '/' },
    { label: 'Routes', href: '/routes' },
    { label: 'By Duration', href: '/routes/by-duration' },
  ];
    ...
    <>
      <section className="relative bg-gradient-to-br from-[var(--primary)]/10 via-[var(--gold)]/5 to-transparent py-12 border-b border-[var(--border)]">
        <div className="container-wide">
          ...
          <p className="text-lg text-[var(--muted)] max-w-3xl mb-4">{entry.meta.description}</p>
        </div>
      </section>
