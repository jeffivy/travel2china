...
export default function ToolsSlugPage({ params }: { params: { slug: string } }) {
  const entry = getContentBySlug('tools', params.slug);
  if (!entry) notFound();
  const breadcrumbs = [
    { label: 'Home', href: '/' }, { label: 'Tools', href: '/tools' },
    { label: entry.meta.title, href: `/tools/${params.slug}` },
  ];
  return (
    <>
      <section className="relative bg-gradient-to-br from-[var(--primary)]/10 via-[var(--gold)]/5 to-transparent py-12 border-b border-[var(--border)]">
        <div className="container-wide">
          <Link href="/tools" className="inline-flex items-center gap-1 text-sm text-[var(--muted)] hover:text-[var(--primary)] transition-colors mb-6"><ArrowLeft className="w-4 h-4" /> All Tools</Link>
      ...
      <article className="container-content py-10">
        <div className="prose dark:prose-invert max-w-none"><MDXContent source={entry.content} /></div>
      </article>
    </>
  );
}
